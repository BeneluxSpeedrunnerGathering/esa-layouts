...
function updateCountdownTimer() {
    const cdTimer = replicants_1.countdown.value;
    const remaining = cdTimer.originalDuration - (Date.now() - cdTimer.timestamp);
    if (remaining > 0) {
        replicants_1.countdown.value.remaining = remaining;
        countdownTimeout = setTimeout(updateCountdownTimer, 1000);
    }
    else {
        replicants_1.countdown.value.remaining = 0;
    }
}
nodecg_1.get().listenFor('startCountdown', (time) => {
