...
async function getEventIDFromShort(short) {
    const resp = await needle_1.default('get', `https://${config.address}/search/?short=${short}&type=event`, cookies);
    if (!resp.body.length) {
...
async function updateDonationTotalFromAPI() {
    try {
        ...
        for (const event of exports.eventInfo) {
            const resp = await needle_1.default('get', `https://${config.address}/${event.id}?json`);
        ...
        if (replicants_1.donationTotal.value < total) {
            nodecg_1.get().log.info('[Tracker] API donation total changed: $%s', total);
            replicants_1.donationTotal.value = total;
        }
    }
    catch (err) {
        nodecg_1.get().log.warn('[Tracker] Error updating donation total from API');
        nodecg_1.get().log.debug('[Tracker] Error updating donation total from API:', err);
    }
}
...
rabbitmq_1.mq.evt.on('donationTotalUpdated', (data) => {
    ...
    if (replicants_1.donationTotal.value < total) {
        nodecg_1.get().log.debug('[Tracker] Updated donation total received: $%s', total.toFixed(2));
        replicants_1.donationTotal.value = total;
    }
});
...
rabbitmq_1.mq.evt.on('donationFullyProcessed', (data) => {
    if (data.comment_state === 'APPROVED') {
        ...
        nodecg_1.get().log.debug('[Tracker] Received new donation with ID %s', data._id);
...
async function loginToTracker() {
    ...
    try {
        ...
        if (isFirstLogin) {
            isFirstLogin = false;
            nodecg_1.get().log.info('[Tracker] Successfully logged in');
        }
        else {
            nodecg_1.get().log.info('[Tracker] Successfully refreshed session');
        }
        // Tracker logins expire every 2 hours (apparently?). Re-login every 90 minutes.
        setTimeout(loginToTracker, 90 * 60 * 1000);
    }
    catch (err) {
        nodecg_1.get().log.warn('[Tracker] Error authenticating');
        nodecg_1.get().log.debug('[Tracker] Error authenticating:', err);
        if (!isFirstLogin) {
...
async function init() {
    if (!eventConfig.shorts.length) {
        nodecg_1.get().log.warn('[Tracker] No events found in configuration to query for');
        return;
    }
    try {
        nodecg_1.get().log.info('[Tracker] Setting up');
        ...
        nodecg_1.get().log.warn('[Tracker] Error setting up');
        nodecg_1.get().log.debug('[Tracker] Error setting up:', err);
    }
}
if (config.enable) {
    init();
